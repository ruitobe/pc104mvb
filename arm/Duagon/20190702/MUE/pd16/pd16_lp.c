 ...
 */
TCN_DECL_CONST \
    UNSIGNED16  pd16_lp_port_number_max     = PD16_LP_PORT_NUMBER_MAX;
TCN_DECL_CONST \
    UNSIGNED16  pd16_lp_port_size_number    = PD16_LP_PORT_SIZE_NUMBER;
TCN_DECL_CONST \
    UNSIGNED8   pd16_lp_port_size_value[16] = PD16_LP_PORT_SIZE_VALUE;
...
#define PD16_LP_MACRO_UART_MUTEX_LOCK(_p_osl_mutex_object_uart_)            \
    pthread_mutex_lock((_p_osl_mutex_object_uart_))

#define PD16_LP_MACRO_UART_MUTEX_UNLOCK(_p_osl_mutex_object_uart_)          \
...
/* --------------------------------------------------------------------------
 *  Procedure : pd16_lp_init (of procedure type 'LP_INIT')
 *
 ...
 *                'pd16_lp_xxx' procedure referencing the
...
)
{
    ...
    PD16_LP_MACRO_UART_MUTEX_LOCK(&p_pd16_bus_ctrl->osl_mutex_object_uart);
    ...
    counter = 0;
    while ((counter < pd16_lp_port_number_max) && (LP_OK == lp_result))
    {
        mue_result =                    \
    ...
    if (LP_OK == lp_result)
    {
        p_pd16_bus_ctrl->lp_fsi = fsi;
    } /* if (LP_OK == lp_result) */

    PD16_LP_MACRO_UART_MUTEX_UNLOCK(&p_pd16_bus_ctrl->osl_mutex_object_uart);
...
/* --------------------------------------------------------------------------
 *  Procedure : pd16_lp_manage (of procedure type 'LP_MANAGE')
 *
...
)
{
    ...
    if (LP_OK == lp_result)
    {
        ...
        while (TRUE == repeat)
        {
            ...
            if (TRUE == repeat)
            {
                ...
                mue_port_handle++;
                if (mue_port_handle == pd16_lp_port_number_max)
                {
                    if (PD_PRT_CMD_CONFIG == command)
                    {
    ...
    if (LP_OK == lp_result)
    {
        ...
        if (PD_PRT_CMD_DELETE == command)
        {
            ...
            {
                /* store port address configuration (0xFFFF=undefined) */
                p_pd16_bus_ctrl->lp_address[mue_port_handle] = 0xFFFF;
                /* store port size configuration (0=undefined) */
                p_pd16_bus_ctrl->lp_size[mue_port_handle]    = 0;
                /* store last port freshness timer (default=0xFFFF) */
                p_pd16_bus_ctrl->lp_fresh[mue_port_handle]   = 0xFFFF;
            } /* if (LP_OK == lp_result) */
        } /* if (PD_PRT_CMD_DELETE == command) */
        ...
        else if (PD_PRT_CMD_STATUS == command)
        {
            ...
            mue_result = mue_pd_16_get_port_config(p_bus_ctrl, \
        ...
        else
        {
            ...
            if (LP_OK == lp_result)
            {
                ...
                {
                    /* clear port value */
                    memset((void*)mue_port_data, 0x00, 32);
...
#endif /* #ifdef PD16_BC_ERRCNT_ALL_PD */
                /* store port address configuration (0xFFFF=undefined) */
                p_pd16_bus_ctrl->lp_address[mue_port_handle] = \
                    port_address;
                /* store port size configuration (0=undefined) */
                p_pd16_bus_ctrl->lp_size[mue_port_handle]    = \
                    prt_attr->port_size;
            } /* if (LP_OK == lp_result) */
        } /* else */
    } /* if (LP_OK == lp_result) */
...
/* --------------------------------------------------------------------------
 *  Procedure : pd16_lp_put_dataset (of procedure type 'LP_PUT_DATASET')
 *
...
/* --------------------------------------------------------------------------
 *  Procedure : pd16_lp_get_dataset (of procedure type 'LP_GET_DATASET')
 *
...
)
{
    ...
    if (LP_OK == lp_result)
    {
        ...
        /* bus controller structure (dynamic values) - LP */
        /* - store port freshness timer                   */
        p_freshness = \
            &((PD16_BUS_CTRL*)p_bus_ctrl)->lp_fresh[mue_port_handle];
        if (mue_status & mue_pd_port_status_sink_mask)
        {
            *p_freshness = 0x0000;
        } /* if (mue_status & mue_pd_port_status_sink_mask) */
        else
        ...
        /* calculate freshness timer */
        if (p_fresh != NULL)
        {
            *(UNSIGNED16*)p_fresh = *p_freshness;
        } /* if (p_fresh != NULL) */
    } /* if (LP_OK == lp_result) */
