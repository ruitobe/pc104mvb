...
)
{
           ...
           (timeout < timeout_max))
    {
        ...
        //MUE_OSL_PRINTF("mue_acc_tx_start reg_lsr = 0x%x\r\n", reg_lsr);
        timeout++;
			  //printf("timer--\n");
			  //delay_ms(1);
    } /* while ((...) && (...)) */
    ...
    if (reg_lsr == mue_acc_uart_reg_lsr_bit_thre)
    {
			  //MUE_OSL_PRINTF("MUE_RESULT_OK reg_lsr = 0x%x\r\n", reg_lsr);
        mue_result = MUE_RESULT_OK;
    } /* if (reg_lsr == mue_acc_uart_reg_lsr_bit_thre) */
    else
    {
			  MUE_OSL_PRINTF("MUE_RESULT_TIMEOUT reg_lsr = 0x%x, timeout_max = %d\r\n", reg_lsr, timeout_max);
        mue_result = MUE_RESULT_TIMEOUT;
			  //mue_result = MUE_RESULT_OK;
    } /* else */
