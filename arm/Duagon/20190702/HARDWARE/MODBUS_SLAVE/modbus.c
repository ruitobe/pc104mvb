...
void Handle_port_config_reg(uint8_t port_index, uint16_t data) {
  //mvb_ts_cfg[port_index].prt_addr = (data & 0xFFF);
	m_md_prt_cfg[port_index].pd_port_address = (data & 0xFFF);
	m_md_prt_cfg[port_index].pd_prt_attr.port_size = port_size[((data & 0x7000)>>12)];
	//mvb_ts_cfg[port_index].size = port_size[((data & 0x7000)>>12)];
	//mvb_ts_cfg[port_index].type = (data & 0x8000)== 0 ? LP_CFG_SINK : LP_CFG_SRCE;
	m_md_prt_cfg[port_index].pd_prt_attr.p_bus_specific = NULL;
	m_md_prt_cfg[port_index].pd_prt_attr.port_config = (data & 0x8000)== 0 ? PD_SINK_PORT : PD_SOURCE_PORT;
	/*if (port_index != 0) {
